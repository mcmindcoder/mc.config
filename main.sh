...
if [ -f "/usr/bin/ansible-playbook" ]; then
  ...
  sudo /usr/bin/python2 /usr/bin/ansible-playbook $ARGS --connection=local main.yml -e 'ansible_python_interpreter=/usr/bin/python2'
  exit
elif [ -f "/usr/local/bin/ansible-playbook" ]; then
	echo Use /usr/local/bin/ansible-playbook
	sudo /usr/bin/python2 /usr/local/bin/ansible-playbook $ARGS --connection=local main.yml -e 'ansible_python_interpreter=/usr/bin/python2'
  exit
else
