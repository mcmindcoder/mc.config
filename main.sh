...
if [ -f "/usr/bin/ansible-playbook" ]; then
	...
	/usr/bin/python3 /usr/bin/ansible-playbook $ARGS --connection=local main.yml -e 'ansible_python_interpreter=/usr/bin/python3'
	exit
elif [ -f "/usr/local/bin/ansible-playbook" ]; then
	sudo echo Use /usr/local/bin/ansible-playbook
	/usr/bin/python3 /usr/local/bin/ansible-playbook $ARGS --connection=local main.yml -e 'ansible_python_interpreter=/usr/bin/python3'
	exit
else
